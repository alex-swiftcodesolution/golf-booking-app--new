    ...
    try {
      ...
      if (guests.length > 0) {
        ...
        const updatedGuestPassesUsed =
          guestData.guestPassesUsed + calculatedGuestPassUsage.free;
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            console.error("Guest data update failed:", error.message);
  ...
  */
  ...
  const addBooking = async (
    ...
    try {
      ...
      const isPaidGuestVisit = servicename
        .toLowerCase()
        .includes("guest paid visit");

      if (isPaidGuestVisit && guestData.purchasedGuestPasses <= 0) {
      ...
      if (guests.length > 0) {
        ...
        const updatedPurchasedGuestPasses = isPaidGuestVisit
          ? Math.max(guestData.purchasedGuestPasses - guests.length, 0) // Deduct for paid guest visits
  ...
  */
  ...
  const deleteBooking = async (id: number, token: string): Promise<void> => {
    try {
      ...
      const booking = bookings.find((b) => b.id === id);
      const isGuestBooking = booking && booking.guests.length > 0;
      const isPaidGuestVisit =
        isGuestBooking && booking.guestPassUsage.charged > 0;
      ...
      if (isGuestBooking) {
        ...
        const updatedPurchasedGuestPasses = isPaidGuestVisit
          ? guestData.purchasedGuestPasses + booking.guestPassUsage.charged // Refund charged passes
          : guestData.purchasedGuestPasses;
